...
- 오케스트레이터: payload 읽고 `plan-harness/references/α-pending-to-question.md` 규약으로 기획자 언어로 질문 → 응답을 `decision:` 필드에 기록 → **다음 라운드의 새 `Agent(bp:planner)` spawn** (체이닝 모델 — 세션 재진입 없이 매번 새 Agent 호출. `convergence-loop.md` 의 "α 결정 반영" prompt 템플릿 사용. `/bp:plan` 세션이 이미 종료된 경우도 동일 경로로 이어감)
...
6. **planner subagent 위임** — 컨펌 후 Agent tool 로 `bp:planner` 에 intake.md 경로 전달 (`Agent(bp:planner, round=1)`). planner 는 feature-spec / screen-spec 호출 + 수렴 루프만 담당
7. **α 재진입 (필요 시)** — planner 가 `status: awaiting_decision` 으로 턴 종료하면 오케스트레이터가 `## _pending_decisions` 읽고 기획자 언어로 질문 → 응답 받아 `decision:` 필드에 기록 → **다음 라운드의 새 `Agent(bp:planner)` spawn** 으로 결정 반영 지시 (체이닝 모델. `/bp:plan` 세션이 이미 종료된 경우도 동일 경로)
