...
- "α 재진입 (옵션 B)" 섹션의 오케스트레이터 후속 조치 기술을 **SendMessage 세션 재진입 → Agent 체이닝 (다음 라운드 새 spawn)** 으로 업데이트. plan-harness 4.0.0 의 체이닝 모델에 맞춤
- "오케스트레이터 사용 흐름" 6·7단계 문구 동일 업데이트

payload 스키마는 무변화 — 기존 intake.md 는 그대로 호환.
