...
• 상품 상세, 구매자용, PC·모바일 둘 다
• 상품 정보 + 옵션 + 장바구니 + 리뷰 요약
• 이미지 확대, 리뷰 더보기 시트, 이미지 확대 다이얼로그
• 비로그인 담기 시 로그인 유도

이 내용으로 '상품' 쪽 기능명세 손보고, '상품 상세' 화면 만들게요.
(만들 파일 목록이 길어 내부에 기록해뒀어요. 확인 원하면 "파일 목록" 이라고 해주세요.)
...
**구조**:
1. "정리했어요" — 개방 인사
2. intake 요약 (3~4줄 bullets, 슬롯 이름 숨김)
3. 작업 자연어 요약 한 줄 (기능명세 도메인명 + 화면 한글명)
4. 파일 목록 이탈 경로 한 줄 ("파일 목록" 이라고 말하면 펼쳐줌)
5. "이대로 진행할까요?" — 짧은 확인

**"파일 목록" 요청 시 펼치는 목록 (기획자가 물었을 때만)**:
...
- 9개 슬롯 이름 그대로 나열
- `status: ready` 노출
- 기본 응답 메시지에서 파일 경로·파일명 모두 나열 (기획자가 요청했을 때만 펼쳐 보여줌)
- 카테고리 라벨 ("기능명세 / 화면") 을 기본 응답에 박는 것 — 기획자에게 생소한 용어. 자연어 "상품 쪽 기능명세" / "상품 상세 화면" 로 풀어 씀
