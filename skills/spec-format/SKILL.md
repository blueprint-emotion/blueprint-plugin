...
| 섹션명 | 목적 | 읽는 주체 |
|--------|------|-----------|
| `비즈니스 로직` | 비즈니스 규칙 — 기능명세의 SSOT. 와이어프레이머는 도메인 맥락 참고용으로 읽되, 구체적 수치/정책은 이 섹션이 원본 | downstream 소비자, wireframer (참고) |
...
## Requirement
### 인증 — @AUTH/LOGIN
- Given 이메일과 비밀번호 입력 완료 When 로그인 버튼 클릭 Then 다음 화면으로 이동한다
- Given 이메일 형식이 유효하지 않음 When 로그인 버튼 클릭 Then 입력 필드에 에러 메시지가 표시된다
...
| 10 | intake 나머지 4개(핵심 행동, 화면 구성, 특수 인터랙션, 제약사항)가 Screen/Requirement/UserStory/열린 이슈에 반영되었는지 자동 대조. 제약사항 중 비즈니스 정책은 기능명세 비즈니스 로직에, UI 제약은 화면명세 Requirement에 반영 |
...
- 처음부터 모든 섹션을 채울 필요 없다. 최소 기준은 `비즈니스 로직` 섹션
