...
명세에서 `sheet_*.md` / `dialog_*.md` 로 분리돼 있어도 **와이어는 기본 메인 wireframe 안에 fragment 로 모두 포함**한다. 별도 파일은 **opt-in** — 오케스트레이터의 Task prompt 에 `extract_overlays: [...]` 로 이름이 명시된 경우에만 생성한다. wireframer 는 자의적으로 별도 파일을 만들지 않는다.

### 판단 주체

- **오케스트레이터** 가 `sheet_*.md` / `dialog_*.md` 를 스캔해 "상태 변형이 많거나 입력 폼이 복잡" 한 overlay 를 골라 **기획자에게 짧게 질문**. 구체 기준·질문 문구는 [`wireframe-harness/confirm-gates.md` 게이트 2](../../wireframe-harness/references/confirm-gates.md#게이트-2--상태-많은-overlay-분리-여부-오케스트레이터가-소유) 소유
- **기획자** 가 결정. 기본 응답(무응답·"그대로") → 메인 포함. 이름 지정 → 그 이름만 별도 파일
- **wireframer** 는 Task prompt `extract_overlays` 에 이름이 있는 overlay 만 별도 파일로 추가 생성. 없는 이름은 메인 only

| 표현 | 언제 |
|------|------|
| 메인 와이어 안 `<bp-fragment>` + `<bp-dialog open>` (기본) | 거의 항상. 트리거 컨텍스트 + "어디서 어떤 오버레이가 뜨는지" 를 검토자가 한눈에 봐야 할 때 |
| 별도 `wireframe_sheet_{name}.html` 파일 (opt-in) | 해당 overlay 자체가 상태 매트릭스(초기·입력중·전송중·에러·결과) 를 풍부하게 담고 있어 메인 안 fragment 하나로는 매트릭스가 읽히지 않을 때 |

두 표현은 공존한다 — 별도 파일을 만들어도 메인 와이어 안 fragment 는 그대로 유지 (트리거 컨텍스트는 메인이 SSOT).
