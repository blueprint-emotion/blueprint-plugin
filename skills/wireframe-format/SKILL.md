...
- `overflow`가 걸리는 컴포넌트 (`<bp-table>`, `<bp-scroll-area>`, `<bp-carousel>`) → 래퍼 `<div>`에 `data-el`. 컴포넌트 자체에 직접 붙이면 하이라이트 outline이 잘린다
...
| `label` | Yes | 한국어 표시명 |
| `description` | Yes | 마크다운 형식의 상세 설명. 아래 가이드 참조 |

### description 작성 가이드

description은 기획자가 요소의 역할과 동작을 이해할 수 있도록 **구체적으로** 작성한다. 지원하는 마크다운 문법은 3가지만: `**bold**`, `- ` 불릿 리스트, `\n` 줄바꿈. 그 외 문법(헤딩, 테이블, 코드블록 등)은 렌더링되지 않는다.

필수 포함 항목 (요소 유형별):

| 요소 유형 | 필수 기술 항목 |
|----------|---------------|
| input | 역할, 입력 형식/제약, 유효성 검증 규칙 |
| select | 역할, 옵션 목록, 기본값 |
| button | 역할, 클릭 시 동작/결과, 비활성 조건 |
| table | 역할, 컬럼 구성, 행 클릭 동작, 정렬/페이징 |
| list | 역할, 항목 구성, 빈 상태 처리 |

작성 양식:

```
**역할**: 이 요소가 하는 일 한 줄 요약
**옵션/형식**: 선택지 또는 입력 형식 (해당 시)
**동작**: 사용자 액션 → 결과
**조건**: 비활성/숨김/에러 조건 (해당 시)
```
...
```json
{
  ...
  "type": "select",
  "label": "문의 유형",
  "description": "**역할**: 문의 유형별 필터링\n**옵션**: 전체(기본), 상품문의, 배송문의, 교환/반품, 기타\n**동작**: 선택 시 해당 유형만 목록에 표시"
}
```
...
```json
{
  ...
  "type": "table",
  "label": "문의 목록 테이블",
  "description": "**역할**: 문의 목록 조회 및 탐색\n**컬럼**: 번호, 유형, 상품명, 문의 내용, 작성자, 공개여부, 답변 상태, 등록일\n**동작**: 행 클릭 시 상세 페이지 이동, 컬럼 헤더 클릭 시 정렬\n**빈 상태**: 조건에 맞는 문의가 없을 때 안내 표시"
}
```
...
- 나머지 자식 → main 슬롯 (`p-6` 패딩 내장. 콘텐츠 래퍼에 `p-6` 중복 금지)
- 모바일은 body에 `class="... max-w-[400px] mx-auto"` 추가

### 고정 영역 (헤더/푸터)

화면 명세에 헤더/푸터에 대한 언급이 없으면 기존 헤더/푸터를 건드리지 않는다. 레이아웃 항목에 "헤더", "푸터"라는 단어가 명시적으로 없으면 header/footer 슬롯은 기본 라벨을 유지한다. 기본은 **라벨만 표시**한다. 커스텀이 필요한 화면에서만 내용을 채운다.
...
- **안내/설명 문구 금지**: 네비게이션 동작, 인터랙션 설명 등을 `bp-alert`나 텍스트로 DOM에 직접 넣지 않는다. 이런 정보는 메타데이터 `elements[].description`에만 기술한다. 와이어프레임은 UI 요소만 렌더링한다
