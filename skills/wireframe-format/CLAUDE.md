...
화면 명세에 헤더/푸터에 대한 언급이 없으면 기존 헤더/푸터를 건드리지 않는다. 레이아웃 항목에 "헤더", "푸터"라는 단어가 명시적으로 없으면 header/footer 슬롯은 기본 라벨을 유지한다. 화면 명세에 명시된 고정 영역은 `data-feature`/`data-el` 없이 `<bp-page>`의 header/footer 슬롯에 렌더링.
