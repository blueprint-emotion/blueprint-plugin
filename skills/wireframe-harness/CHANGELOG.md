...
### Changed — 게이트 2 발동 조건과 질문 문구 단순화

`references/confirm-gates.md` 게이트 2 를 "`sheet_*.md` / `dialog_*.md` 가 있으면 항상 발동" 에서 "**상태 변형이 많거나 입력 폼이 복잡한** overlay 가 있을 때만 발동" 으로 좁힘. 오케스트레이터가 미리 파일을 스캔해 자동 판단.

**왜**: 기존 문구는 모든 overlay 를 나열하면서 "(입력 폼이 풍부하거나 상태 변형이 많은 시트면 추천)" 같은 판단 힌트를 기획자에게 노출해 질문이 무거웠다. 실제 사례(로그인의 `dialog_find_password`) 에서 wireframer 가 이 힌트를 자기 판단 권한으로 해석해 게이트 우회 가능성 확인됨. 기획자 피드백: "기본은 모두 메인 포함. 에이전트가 판단해서 분리 필요하다 싶으면 그때 단순하게 물어봐".

**게이트 2 신 문구**:
```
`sheet_qna_write` 는 상태가 좀 많은데, 별도 파일로 뺄까요?
(기본은 메인에 같이 둡니다. "그대로" 하시면 메인에 포함.)
```

**자동 발동 기준** (오케스트레이터 내부, 기획자 미노출):
- 상태 변형 3개 이상
- 입력 필드 5개 이상
- 기획자가 명세에 "별도 파일" 을 암시한 경우

기본값 처리 유지 (무응답·"그대로" → 메인 포함).

**추가 — 기획자 facing 메시지에서 overlay 파일명 노출 금지**: 게이트 1·2 와 `wireframer-ux.md` 금기 목록에 규칙 추가. `sheet_qna_write` 같은 파일명 대신 각 파일 frontmatter `title` 에서 카테고리 접두사(`시트_`, `다이얼로그_`, `알랏_`) 떼어 "Q&A 작성 시트" 같은 한글로 호칭. 기획자 응답은 오케스트레이터가 다시 파일명으로 매핑해 `extract_overlays` 에 담음.

호환성: 기존 `extract_overlays` Task prompt 필드·응답 처리 표 동일. 문구만 단순화, 발동 조건만 좁힘, overlay 호칭 방식만 한글화.
