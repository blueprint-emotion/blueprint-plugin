...
description: >
  ...
  까지의 실행 흐름과 "기획자 경험" 원칙을 함께 정의한다.
  Anthropic 공식 제약(subagent 는 Task tool 없음)에 따라 Task(bp:reviewer) 호출과
  SendMessage(planner) 재진입은 모두 오케스트레이터가 전담한다.
...
- `bp:planner` agent 의 명세 작성 + SendMessage 재진입 처리
...
즉 planner subagent 는 Task tool 자체를 갖지 않는다. `ToolSearch(query: "select:Task")` 를 해도 스키마가 로드되지 않아 reviewer 를 spawn 할 수 없다. **Task(bp:reviewer) 호출과 SendMessage(planner) 재진입은 모두 오케스트레이터가 전담**한다.
...
[오케스트레이터가 수행]
 ...
 4. intake + 작업 트리 확정 (사용자와 자연스럽게 확인)
       ↓
 5. Task(bp:planner) — 최초 명세 작성 위임. planner 는 작성 후 반송
       ↓
[오케스트레이터가 수행 — 수렴 루프 주도]
 6. Task(bp:reviewer) — 검수 (매 라운드 새 spawn)
 7. 위반 있으면 SendMessage(planner_id) — 위반 반영 요청
 8. 루프 반복 (최대 3회)
 9. α 재진입 발생 시 기획자 질문 → 응답 → SendMessage(planner) 로 결정 반영
10. 결과 보고 후 종료
```
...
- Task 호출 허브만 오케스트레이터가 맡음 — 공식 제약 때문에 다른 선택이 없고, B1 모델의 "오케스트레이터 = 기획자 facing 대화 소유" 와도 일관

## 핵심 패턴 — Producer-Reviewer (오케스트레이터 주도)

Producer-Reviewer 수렴 루프는 **오케스트레이터가 소유**한다. planner 는 "명세 작성" 과 "SendMessage 로 들어온 위반 반영" 두 가지만 담당.

```
   [오케스트레이터]                 [Producer = planner]            [Reviewer]
                                  
  Task(bp:planner) ─────────────▶  명세 작성
                   ◀──────── 반송 ┘
                                  
  Task(bp:reviewer) ────────────────────────────────────▶  feature/screen 위반 검사
                   ◀────────────────── 위반 리포트 ──────┘
                                  
  SendMessage(planner) ────────▶  자동 수정 가능 → Edit
                                  자동 수정 불가 → [α 프로토콜]
                                                   intake.md 에 awaiting_decision + 
                                                   pending_decisions 기록 후 턴 종료
                   ◀──────── 반송 ┘
                                  
   [α 발생 시]
  기획자 질문 → 응답 → decision Edit
  SendMessage(planner, 결정 반영 지시) ▶ planner 가 Edit 후 반송
                                  
  LOOP 최대 3회. 위반 0건이면 종료.
```

α 재진입 프로토콜 상세는 [convergence-loop.md](references/convergence-loop.md) 참조.
...
**금지어** (기획자 facing 메시지에): 컨펌, 게이트, status, 슬롯, payload, awaiting, pending, frontmatter, 필드, 카테고리, 분류, Task, SendMessage, subagent
...
`/bp:plan` 재호출 시 기술 상태를 들이대지 말고 기획 맥락으로 복기. [resume.md](references/resume.md) 참조.
...
1. **self-check 로 reviewer 호출 대체 금지** — 오케스트레이터는 반드시 Task tool 로 `bp:reviewer` 를 호출. subagent 가 "Task 없다 → advisor 로 대체" 같은 우회 시도도 금지 (task-tool-invocation.md §D)
2. **subagent 가 Task 호출 시도 금지** — planner·wireframer subagent 에 Task tool 이 없음 (공식 제약). "Task 스키마 로드 실패 → 오케스트레이터 반송 + 종료" 가 유일 정답
...
5. **자동 수정은 분류 기반** — reviewer 리포트의 "자동 수정 가능" 레이블이 있어야 planner 가 Edit. 오케스트레이터가 SendMessage 로 리포트 전문을 전달해야 함 (요약·재포맷 금지)
...
- **[convergence-loop.md](references/convergence-loop.md)** — 오케스트레이터 주도 수렴 루프 알고리즘, α 재진입 프로토콜, 최종 보고 포맷
- **[auto-fix-policy.md](references/auto-fix-policy.md)** — feature/screen 위반 분류 기준
- **[resume.md](references/resume.md)** — 중단된 /bp:plan 재진입 (상태별 복기 톤)
- **[task-tool-invocation.md](references/task-tool-invocation.md)** — 오케스트레이터의 Task/SendMessage 호출 절차, prompt/message 템플릿
...
오케스트레이터가 reviewer 를 Task 로 호출할 때 prompt 상단에 한 줄 포함:
...
MAJOR bump 는 Producer-Reviewer 책임 분배·α 재진입 프로토콜·기획자 UX 원칙의 구조 변경을 의미. 이 중 하나라도 바뀌면 agent 본문·command 본문을 함께 검토해야 함.

3.0.0 은 Anthropic 공식 제약(subagent 재귀 spawn 불가) 에 맞춰 **Producer-Reviewer 수렴 루프를 planner 주도 → 오케스트레이터 주도로 이관**한 MAJOR 전환. 이전 버전의 "planner→reviewer Task 호출" 설계는 플랫폼 단에서 작동하지 않던 것.
