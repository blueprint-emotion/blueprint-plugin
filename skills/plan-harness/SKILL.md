...
판정 기준:
- **구조 변경**: TOC 항목 추가/삭제/이름변경, featureId 경로 변경, 기능 헤딩(H2~H4) 추가/삭제
- **표현 수정 only**: 설명 문구 보완, Requirement/UserStory 문장 다듬기, 비즈니스 로직 텍스트 수정, 기존 섹션 내부 텍스트 수정
...
- S1~S8: frontmatter, TOC-본문 일치, 기능 콘텐츠 존재 (비즈니스 로직 또는 하위 기능), 레이아웃 참조, features 배열, INDEX.md 등
...
- S12: intake 나머지 4개(핵심 행동, 화면 구성, 특수 인터랙션, 제약사항)가 명세에 반영되었는지 자동 대조. 제약사항 중 비즈니스 정책은 기능명세에, UI 제약은 화면명세 Requirement에 반영. 정책 미확정이면 명세에도 열린 이슈/가정으로 남아 있어야 함
...
| 어떤 화면이 렌더링 가능한 기능을 참조하지 못할 때 | reviewer (S3, S4) | 하네스 | S3: 기능에 비즈니스 로직 또는 하위 기능 존재, S4: 레이아웃 참조 기능 존재 |
