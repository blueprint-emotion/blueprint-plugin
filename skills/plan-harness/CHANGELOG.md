...
### Changed — 기획자 facing 메시지 경량화·비기술화

기획자 피드백: "인터뷰에 나오는 문구가 너무 많고 기술적이야. 이거 사용하는 사람이 기획자라서 기술용어에 좀 어두워". 가장 무거운 3개 지점을 리라이트.

**① 통합 확인 게이트 (`references/confirm-gates.md`)**
- 파일 목록을 기본 응답에서 감춤. "'상품' 쪽 기능명세 손보고, '상품 상세' 화면 만들게요" 로 자연어 요약 한 줄 + "파일 목록" 요청 시에만 펼치는 이탈 경로
- 카테고리 라벨 ("기능명세 / 화면") 을 기본 응답에서 제거 → 도메인명·화면 한글명으로 풀어 씀
- 15줄 → 8줄

**② α 질문 템플릿 (`references/α-pending-to-question.md`)**
- trade_off 부연 bullet 2줄 → 결과 귀결 1줄 ("공통 규칙이면 한 번만 쓰면 되고, 이 화면 전용이면 여기만 쓸게요")
- `conversation_hint` 인용·`recommended: true` 추천 한 줄은 **필요할 때만** — 매번 박지 않기
- 10줄 → 6줄

**③ 인터뷰 중 확인+질문 (`references/interview-flow.md`)**
- "확인 겸 몇 가지만 더:" + 번호 매긴 3개 질문 → **한 문장 묶음 질문** ("맞아요? 그리고 리뷰·Q&A·추천 상품 영역은 있어요 없어요?")
- 선택지 괄호 (평점만 / 개별 / 안 함) 같은 세부는 후속 1턴으로 미룸 — 한 번에 너무 많이 묻지 않기
- 10줄 → 7줄

**왜**: 이전 템플릿들도 `planner-ux.md` 의 "짧게, 비기술적" 원칙을 따른다고 명시돼 있었으나 **실제 예시 메시지 자체가** 파일명·카테고리 라벨·선택지 세부 까지 한 번에 노출해 기획자 눈엔 여전히 무거움. 원칙과 예시의 간극을 좁히는 작업.

호환성: α payload 필드·응답 처리 표·슬롯 커버리지 내부 규약 동일. 기획자 facing 예시 문구만 경량화.
