...
"같은 옵션은 합쳐진다" 이 규칙, 이 화면에만 쓰는 거예요,
아니면 상품 전반에 다 쓰는 거예요?

공통 규칙이면 한 번만 쓰면 되고, 이 화면 전용이면 여기만 쓸게요.
```

**예시 해설** (참고용, 출력 X):
- 첫 줄: `planner_context` 를 "영역 정리하다가 하나만 짚어볼게요" 수준으로 가볍게
- 가운데: `user_facing_why` 를 **질문 문장 그대로** 로 변형 (trade_off 설명 제거 — 선택의 결과만 뒤에 한 줄)
- 마지막 줄: "A 면 …, B 면 …" 한 줄로 귀결. 선택지 bullet 2줄 + trade_off 부연 2줄 → 1줄로 축약
- `conversation_hint` (조금 전 언급했던…) 는 인용이 자연스러울 때만 (매번 넣지 말 것 — 대화 늘어짐)
- `recommended: true` 의 추천 한 줄도 **선택지가 명백할 땐 생략**. 애매한 결정에만 "제 느낌엔 …" 한 줄
