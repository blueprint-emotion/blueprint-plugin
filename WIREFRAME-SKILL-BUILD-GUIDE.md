...
interface FlowFrameMetaFeature {
  id: string;          // 기능 ID (SCREEN_ID_FEATURE_xxx 규칙)
...
- `features[].id`: `{SCREEN_ID}_FEATURE_{이름}` (예: `LOGIN_FEATURE_EMAIL`, `LOGIN_FEATURE_SUBMIT`)
...
- `features[].id`: `{SCREEN_ID}_FEATURE_{기능명}` 패턴
- 예: screenId가 `LOGIN`이면 → `LOGIN_FEATURE_EMAIL`, `LOGIN_FEATURE_SUBMIT`
