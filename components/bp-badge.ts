...
class BpBadge extends HTMLElement {
  connectedCallback() {
    ...
    this.classList.add(...cn(base, variants[variant] || variants.default).split(" "));
    this.style.display = "inline-flex";
    // Fix border-transparent: the global `* { border-color: var(--border) }` overrides
    // Tailwind's border-transparent, so we set it as inline style.
    // Skip for 'outline' variant which uses border-border intentionally.
    if (variant !== "outline") {
