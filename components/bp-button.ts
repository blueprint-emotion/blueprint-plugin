...
class BpButton extends HTMLElement {
  connectedCallback() {
    ...
    // Apply data-slot and classes directly on the custom element
    ...
    // border-transparent fix: use inline style instead of class to avoid base.css override
    // Only for variants that don't set their own border color (outline sets border-border)
    if (variant !== "outline") {
