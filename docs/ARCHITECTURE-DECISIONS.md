...
화면 md는 **레이아웃 + 기능 참조**만 담는다. 기능의 상세 명세는 features/ 에 둔다.
...
| **기능 명세 (features/*.md)** | 상태, 인터랙션, 비즈니스 로직, API 명세 | 디자이너, 개발자 |
| **화면 정의 (screens/*.md)** | 화면 목적, 레이아웃, 기능 조합 | 기획자 |
...
| 구분 | 역할 | 담당 |
|------|------|------|
| **에이전트** | 기획자와 대화하며 features/*.md, screens/*.md 작성/수정 | 기획 도메인 지식, 템플릿 가이드 |
| **스킬** | md를 읽고 wireframes/*.html 생성/업데이트 | FlowFrame 규격, HTML 렌더링 규칙 |
...
Pass 2: feature md를 하나씩 읽기 → 해당 위치에 와이어프레임 요소 렌더링
  → features/auth.md의 "와이어프레임 요소" 읽기 → 해당 영역 그리기
  → features/comments.md의 "와이어프레임 요소" 읽기 → 해당 영역 그리기
  → ... (한번에 하나씩, 필요한 섹션만)
```
...
기획자: "댓글이랑 인증 수정했어, 와이어프레임 업데이트해줘"
   ↓
에이전트: features/comments.md, features/auth.md 찾기
        → 각 파일의 usedIn 확인
   ↓
에이전트: 영향 범위 표시 + 컨펌 요청
...
기획자: "LOGIN은 빼고 나머지만"
   ↓
에이전트 → 스킬 호출:
  1. features/comments.md의 "와이어프레임 요소" 읽기
  2. features/auth.md의 "와이어프레임 요소" 읽기
...
- `screens/LOGIN.md` → `wireframes/LOGIN.html` (1:1 매칭)
...
| 스킬 | 역할 | 트리거 예시 |
|------|------|------------|
| **flowframe-spec** | 기획자와 대화하며 features/*.md, screens/*.md 작성/수정 | "기획서 만들어줘", "기능 추가", "화면 명세" |
| **flowframe-wireframe** | md를 읽고 wireframes/*.html 생성/업데이트 | "와이어프레임 만들어줘", "와이어프레임 업데이트" |
