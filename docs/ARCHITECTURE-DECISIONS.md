...
```json
{
  "id": "LOGIN_FEATURE_AUTH",
  "type": "input",
  "label": "인증 폼",
  "description": "이메일/비밀번호 로그인 및 소셜 로그인",
...
| 메서드 | 엔드포인트 | 설명 |
|--------|-----------|------|
| GET | /api/comments?screenId={id} | 댓글 목록 |
...
| 필드 | 필수 | 설명 |
|------|------|------|
| `featureId` | Yes | 기능 고유 ID (대문자, 언더스코어). HTML의 `data-feature` 접두사로 사용 |
| `label` | Yes | 기능 이름 |
...
| **와이어프레임 요소** | 에이전트 (와이어프레임 렌더링 시) | HTML의 data-feature 영역 생성 |
| **상태** | 디자이너 | 상태별 UI 디자인 |
| **인터랙션** | 디자이너 + 개발자 | 동작 설계 및 구현 |
| **비즈니스 로직** | 개발자 | 백엔드 로직 구현 |
| **API** | 개발자 | API 설계 및 연동 |
...
- feature md의 `featureId` → HTML의 `data-feature` 속성으로 매칭
...
| 질문 | 배경 |
|------|------|
| PC/모바일 뷰 처리 | 한 화면 md에서 viewport 분기 vs 별도 화면 md |
...
| 에이전트 구현 방식 | AGENTS.md 기반 vs 별도 에이전트 패키지 |
