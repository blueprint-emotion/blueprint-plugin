...
```
features/*.md   → 비즈니스 기능 단위 명세 (재사용 가능, 여러 화면에서 참조)
screens/*.md    → 화면 레이아웃 + 기능 참조만 ([@feature](../features/feature.md) 링크)
wireframes/*.html → 생성된 HTML 와이어프레임 (FlowFrame 업로드용)
```
...
- **추적 ID 패턴**: `FEATURE_{FEATURE_ID}_{ELEMENT_NAME_IN_ENGLISH}` (`featureId` + 요소 영문명 조합, 메타데이터 `elements` 항목과 HTML `data-feature`에 사용)
- **data-feature 속성**: 모든 추적 요소에 필수, 메타데이터 `elements` 배열과 1:1 매칭
...
| 샘플 HTML | `samples/LOGIN.html` |
| 세션 히스토리 | `docs/history/` |

## Current Status

v2.0 스킬 완성, skill-creator 분석으로 식별된 10개 이슈 수정 중. 상세: `docs/history/`
