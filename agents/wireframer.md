...
3. HTML 파일 작성 (`docs/screens/{SCREEN_ID}/{screenId 소문자}_wireframe.html`):
   - `<head>`: Tailwind CDN, `bp-platform.js` 스크립트, 다크모드 설정
...
모달 파일도 메인 와이어프레임과 동일한 Blueprint 메타데이터 구조를 사용한다.
...
9. 중요 상태가 가시적으로 표현됨
10. Tailwind CDN + `bp-platform.js` 포함, `<style>`에 다크모드 설정 + 최소 커스텀 CSS만 (호버/상태 탭 CSS 금지)
11. 모든 색상에 `dark:` 변형
12. 명세의 핵심 기능 누락 없음
