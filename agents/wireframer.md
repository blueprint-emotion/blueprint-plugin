...
3. HTML 파일 작성 (`docs/screens/{SCREEN_ID}/{screenId 소문자}_wireframe.html`):
   - `<head>`: Tailwind CDN, `flowframe.js` 스크립트, 다크모드 설정
...
2. 모달별 파일: `{screenId 소문자}_modal-{name}.html` — 모달 전체 UI
   - `{name}`은 레이아웃의 모달 항목명을 소문자+하이픈으로 정규화한 슬러그 (예: "저장 버전 선택" → `save-version`, "업로드 확인" → `upload-confirm`)
...
- `docs/screens/{SCREEN_ID}/{screenId 소문자}_wireframe-pc.html` — `### 레이아웃 (PC)`에서
- `docs/screens/{SCREEN_ID}/{screenId 소문자}_wireframe-mobile.html` — `### 레이아웃 (Mobile)`에서
...
9. 중요 상태가 가시적으로 표현됨
10. Tailwind CDN + `flowframe.js` 포함, `<style>`에 다크모드 설정 + 최소 커스텀 CSS만 (호버/상태 탭 CSS 금지)
11. 모든 색상에 `dark:` 변형
12. 명세의 핵심 기능 누락 없음
